...
#define _DUTY_MAX 2550 // 2000
#define _DUTY_NEU 1679 // 1500
#define _DUTY_MIN 600 // 1000

#define _SERVO_ANGLE_DIFF  7  // Replace with |D - E| degree
#define _SERVO_SPEED       800  // servo speed 

// Target Distance
#define _DIST_TARGET    200 // Center of the rail (unit:mm)
...
void loop() {
  ...
  if (event_dist) {
   ...
   // PID control logic
    ...
    if((dist_ema>165)&&(dist_ema<185)) {
      dterm = 250 * (error_current - error_prev);
      pterm = 2 * error_current;
    }
    else {
      dterm = _KD * (error_current - error_prev);
      pterm = _KP * error_current;
...
float volt_to_distance(int a_value)
{
  ...
  return 1240 + -6.96*a_value + 0.0138*pow(a_value,2) + -0.00000934*pow(a_value,3);
}
